...
contract BeraSleepFlexiblePool is Ownable, Pausable {
    ...
    IBeraSleepPool public immutable beraSleepPool; //BeraSleep pool
    ...
    //When call beraSleeppool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositBeraSleep(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _beraSleepPool: BeraSleepPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_beraSleepPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the BeraSleep Flexible Pool.
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.beraSleepAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    beraSleepPool.withdrawByAmount(withdrawAmount);
        ...
        user.beraSleepAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositBeraSleep(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the BeraSleep Flexible Pool
    ...
    /**
     * @notice Withdraws from BeraSleep Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking beraSleep");
        staking = false;
        beraSleepPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the BeraSleep Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in BeraSleepPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = beraSleepPool.userInfo(address(this));
        uint256 pricePerFullShare = beraSleepPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into BeraSleepPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            beraSleepPool.deposit(bal, 0);
        }
    }
}
